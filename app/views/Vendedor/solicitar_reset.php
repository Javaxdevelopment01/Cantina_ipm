<?php
$tipo = $_GET['tipo'] ?? 'vendedor';
$bgClass = ($tipo === 'admin') ? 'background: linear-gradient(135deg, #000000 0%, #333333 100%);' : 'background: linear-gradient(135deg, #012E40 0%, #1a4d66 100%);';
$backLink = ($tipo === 'admin') ? '../adm/login_adm.php' : 'login_vendedor.php';
...
<head>
    ...
    <title>Solicitar Redefinição - <?= $titulo ?></title>
    <style>
        body { font-family: 'Segoe UI', sans-serif; display: flex; justify-content: center; align-items: center; min-height: 100vh; <?= $bgClass ?> margin: 0; }
...
<body>
    <div class="card">
        <h2>Redefinir Senha (<?= $titulo ?>)</h2>
        <p>Preencha os dados abaixo. Sua nova senha será ativada após a <strong>aprovação de um Administrador</strong>.</p>
        ...
        <form id="resetRequestForm">
            ...
            <input type="password" name="new_password" placeholder="Nova Senha (min. 6 caracteres)" required minlength="6">
        ...
        <a href="<?= $backLink ?>" class="back-link">Voltar para Login</a>
    </div>
    ...
    <script>
        document.getElementById('resetRequestForm').addEventListener('submit', async function(e) {
            ...
            const p1 = this.querySelector('[name=new_password]').value;
            ...
            try {
                ...
                const response = await fetch('../../api/registrar_pedido_reset.php', {
                    method: 'POST',
                    body: formData
                });
                ...
                btn.disabled = false;
                btn.textContent = 'Solicitar Redefinição';
            }
        });
    </script>
</body>
